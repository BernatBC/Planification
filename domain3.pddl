(define (domain expert-logistic)
    ...
    (:action entrar
        ...
        :effect (and
            (servido ?loc)
            (when (low-petition ?loc ?b) (decrease (sum-petitions) 3))
            (when (medium-petition ?loc ?b) (decrease (sum-petitions) 2))
            (when (important-petition ?loc ?b) (decrease (sum-petitions) 1))
            (not (esta-en ?loc ?b))
        )
    )
